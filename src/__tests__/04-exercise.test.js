...
describe("04-exercises", () => {
  test("createUserObject returns a user object with the right properties", () => {
   ...
   const {firstName, lastName, email} = createUserObject('Alex', 'Spence', 'alex@mail.com')
  ...
  test("createCorporateEmailAddress appends the corporate email domain", () => {
    ...
    // Write the assertion
    const isValid =  'dani@company.com'
    expect(createCorporateEmailAddress('dani')).toMatch(isValid)
  });
